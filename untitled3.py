...
total=list(zip(pmid_dict,contador,ad_dict,dp_dict))

if tipo=='AU'
    pd.DataFrame(total(1,2)),columns=['PMID','num_auth'])
elif tipo=='AD'
    pd.DataFrame(list(zip(ad_dict,contador)),columns=['country','num_auth'])
elif tipo=='DP'
    pd.DataFrame(list(zip(pmid_dict,dp_dict)),columns=['PMID','DP_year'])
